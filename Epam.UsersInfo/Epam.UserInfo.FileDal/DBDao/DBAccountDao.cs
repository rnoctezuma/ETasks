...
namespace Epam.UserInfo.Dal.DBDao
{
    public class DBAccountDao : IAccountDao
    {
        ...
        public IEnumerable <Account> GetAll()
        {
            ...
            {
                SqlCommand cmd = connection.CreateCommand();
                cmd.CommandText = "SELECT db.Account.ID,db.Account.Login, db.Role.RoleName FROM db.Role INNER JOIN db.Account ON db.Role.ID=db.Account.RoleID";
                connection.Open();
                ...
                while (reader.Read())
                {
                    ...
                    yield return new Account(Id, login, roleName);
                }
            }
        }
